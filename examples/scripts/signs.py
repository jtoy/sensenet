...
p.connect(p.GUI)
#p.connect(p.DIRECT)
obj_to_classify = p.loadURDF("mesh.urdf",(0,-1,0),useFixedBase=True)
#p.setDebugObjectColor(obj_to_classify,-1,[255,0,0,1])
p.changeVisualShape(obj_to_classify,-1,rgbaColor=[1,0,0,1])
...
while (1):
  ...
  p.setJointMotorControl2(hand,32,p.POSITION_CONTROL,ring)
  p.setJointMotorControl2(hand,34,p.POSITION_CONTROL,ring)
  p.setJointMotorControl2(hand,36,p.POSITION_CONTROL,ring)
